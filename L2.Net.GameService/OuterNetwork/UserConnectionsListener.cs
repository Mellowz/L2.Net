...
namespace L2.Net.GameService.OuterNetwork
{
    ...
    internal static class UserConnectionsListener
    {
        ...
        {
            Logger.WriteLine(Source.OuterNetwork, "Client connections listener initialized on {0}", m_LocalEndPoint);
        }
        ...
        {
            foreach ( UserConnection connection in m_ActiveConnections.Values.ToArray() )
            {
                connection.CloseConnection();
