...
namespace L2.Net.NpcService.Network
{
    ...
    /// </summary>
    internal static class GameServiceConnection
    {
        /// <summary>
        ...
        {
            Logger.WriteLine(Source.InnerNetwork, "Connected to remote service on {0}", endPoint.ToString());
            Logger.WriteLine(Source.InnerNetwork, "Requesting authentication...");

            Connection.Send
                (
                    new InitializeRequest(Settings.Default.ServiceUniqueID, ( byte )ServiceType.NpcService).ToPacket()
                );
